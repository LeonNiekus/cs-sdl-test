...
namespace CS_SDL_test.Lib.API
{
    ...
    {
        private static bool _anyKeyDown = false;

        static InputState[] keyState = new InputState[(int)KeyCode.KEYCODE_END];
        static InputState[] mouseState = new InputState[(int)MouseButton.MOUSE_BUTTON_END];
        ...
        public struct InputState
        {
            ...
            public InputState(bool down = false, bool transitioned = false)
            {
                this.down = down;
        ...
        public static void init_listeners()
        {
            EventManager.Instance.register_event_handler(
                EventType.KEY_DOWN,
                new Events.EventCallback((Events.Event e) => 
                {
                    var ev = (Events.KeyEvent) e;
                    bool old_state = keyState[(int)ev.key].down;
                    keyState[(int)ev.key].down = true;
                    keyState[(int)ev.key].transitioned = !old_state;
                })
            );
            ...
            EventManager.Instance.register_event_handler(
                ...
                new Events.EventCallback((Events.Event e) =>
                {
                    var ev = (Events.KeyEvent)e;
                    bool old_state = keyState[(int)ev.key].down;
                    keyState[(int)ev.key].down = false;
                    keyState[(int)ev.key].transitioned = old_state;
                })
            );
            ...
            EventManager.Instance.register_event_handler(
                ...
                new Events.EventCallback((Events.Event e) =>
                {
                    var ev = (Events.MouseEvent)e;
                    bool old_state = mouseState[(int)ev.button].down;
                    mouseState[(int)ev.button].down = true;
                    mouseState[(int)ev.button].transitioned = !old_state;
                })
            );
            ...
            EventManager.Instance.register_event_handler(
                ...
                new Events.EventCallback((Events.Event e) =>
                {
                    var ev = (Events.MouseEvent)e;
                    bool old_state = mouseState[(int)ev.button].down;
                    mouseState[(int)ev.button].down = false;
                    mouseState[(int)ev.button].transitioned = old_state;
                })
            );
        }
        ...
        {
            for (int i = 0; i < (int)MouseButton.MOUSE_BUTTON_END; ++i)
                if (InputEvents.get_mouse_button_state((MouseButton)i))
                    return true;

            for (int i = 0; i < (int)KeyCode.KEYCODE_END; ++i)
                if (InputEvents.get_key_state((KeyCode)i))
                    return true;
        ...
        {
            bool currentAnyKeyDown = any_key();

            if (!_anyKeyDown && currentAnyKeyDown)
            {
                _anyKeyDown = currentAnyKeyDown;
                return true;
            }
        ...
        public static bool get_key(KeyCode key)
        {
            return InputEvents.get_key_state(key);
        }

        public static bool get_key_down(KeyCode key)
        {
            return get_key_state(key, true);
        }

        public static bool get_key_up(KeyCode key)
        {
            return get_key_state(key, false);
        }

        public static bool get_mouse_button(MouseButton button)
        {
            return InputEvents.get_mouse_button_state(button);
        }

        public static bool get_mouse_button_down(MouseButton button)
        {
            return get_mouse_button_state(button, true);
        }

        public static bool get_mouse_button_up(MouseButton button)
        {
            return get_mouse_button_state(button, false);
        }

        private static bool get_key_state(KeyCode key, bool expected_state)
        {
            if ((expected_state == keyState[(int)key].down) && keyState[(int)key].transitioned)
            {
                keyState[(int)key].transitioned = false;
                return true;
            }
            return false;
        }

        private static bool get_mouse_button_state(MouseButton button, bool expected_state)
        {
            if ((expected_state == mouseState[(int)button].down) && mouseState[(int)button].transitioned)
            {
                mouseState[(int)button].transitioned = false;
                return true;
            }
            return false;
        }
